...
| Tool | What it does |
| --- | --- |
| `fs_read` | Read files / globs, multiple in one call; supports `offset`/`limit` line ranges |
| `find` | Search files for text/regex — returns path, line number and the line content |
| `fs_edit` | Multiple surgical find-and-replace edits across files in one call |
...
(`.github/workflows/publish-connector.yml`): push a `connector-v*` tag and the
workflow bumps the version from the tag and publishes (needs an `NPM_TOKEN`
secret in the repo).
