...
function serve({ base, token, code, name, mode = "allow", cwd, onAuthToken, print = console.log }) {
  ...
  return new Promise((resolve, reject) => {
    const connect = () => {
      ...
      socket.on("open", () => {
        backoff = 1000;
        print("\n[StarHostAI] Connected — the AI can now use files & terminal on this machine.");
        print(`[StarHostAI] Mode: ${mode}. Ctrl+C to disconnect.\n`);
      });
      ...
      socket.on("message", async (raw) => {
        ...
        if (msg.type === "tool_call") {
          ...
          const result = await execute(tool, args || {}, { mode, cwd, approved: Boolean(approved) });
          send({
            type: "result",
            requestId,
            ok: result.ok,
